...
public class ServiceDescriptionUpdater {
	...
	public void refreshSwaggerConfigurations(){
		...
		//Parent Swagger
		Optional<Object> jsonDataParent = getSwaggerDefinitionForAPI("auth", "http://localhost:8080/v2/api-docs");
		if(jsonDataParent.isPresent()){
			String content = getJSON("auth", jsonDataParent.get());
			definitionContext.addServiceDefinition("auth", content);
		}else{
			logger.error("Skipping service id : {} Error : Could not get Swagegr definition from API ","Auth");
		}
		//Parent Swagger
